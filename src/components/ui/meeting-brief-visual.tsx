...
export function MeetingBriefVisual({ className }: MeetingBriefVisualProps) {
    ...
    const animationStateRef = useRef<'idle' | 'running' | 'paused'>('idle')
    ...
    const typeText = useCallback((
        ...
        function typeNextChar() {
            // Only continue if animation is still running and page is visible
    ...
    const startMeetingAnimation = useCallback(() => {
        // Only start if conditions are right
        if (!isPageVisible || document.hidden || !inView) {
            return
        }
    ...
    }, [currentMeetingIndex, typeText, typeKeyPoints, isPageVisible, inView])
    ...
    useEffect(() => {
        function handleVisibilityChange() {
            ...
            if (!isVisible) {
                ...
                // Page visible again and component in view - restart cleanly
                resetAnimationState()
    ...
    }, [inView, resetAnimationState, startMeetingAnimation])
    ...
    // Main effect for starting/stopping animations
    useEffect(() => {
        if (inView && isPageVisible && !document.hidden) {
            ...
            // Component out of view or page hidden - clean stop
            if (animationStateRef.current === 'running') {
                animationStateRef.current = 'idle'
                clearAllTimeouts()
            }
        }
    }, [inView, currentMeetingIndex, startMeetingAnimation, isPageVisible])
                                    ...
                                    >
                                        <div className="relative mt-0 flex justify-center w-3">
                                            <div
                                                ...
                                                aria-hidden="true"
                                            >
                                                {isCurrent && isPageVisible && animationStateRef.current === 'running' && (
                            ...
                            {isContextExpanded && (
                                <div id="historical-context-content" className="px-3 pb-3">
                                    <p className="text-sm text-emerald-300/90 leading-relaxed min-h-[50px]">
                                        {displayedContext}
                                        {displayedContext && displayedContext !== EXAMPLE_MEETINGS[currentMeetingIndex].historicContext && isPageVisible && animationStateRef.current === 'running' && (
                                    ...
                                    {displayedKeyPoints.map((point, index) => (
                                        ...
                                        >
                                            ...
                                            {index === displayedKeyPoints.length - 1 &&
                                                ...
                                                isPageVisible && animationStateRef.current === 'running' && (
...
// Static data moved to bottom per conventions
